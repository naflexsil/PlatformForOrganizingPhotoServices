...
const DealsTab = () => {
    ...
    fetch("/api/deals", { headers: { Authorization: `Bearer ${accessToken}` } })
      ...
      .then((data) => {
        if (data.status === "success") {
          const active = data.data.filter((d) => ACTIVE.includes(d.status));
          const finished = data.data.filter((d) => !ACTIVE.includes(d.status));
          setDeals([...active, ...finished]);
        }
      })
      .catch(() => {})
  ...
  if (isLoading) {
    return <div className={s.list}>{[1, 2].map((i) => <div key={i} className={s.skeleton} />)}</div>;
  }
  ...
  return (
    <>
      <div className={s.list}>
        ...
        {finished.length > 0 && (
          <>
            <div className={s.section}>Завершенные</div>
